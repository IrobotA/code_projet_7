...
id_list = pd.read_csv('application_test.csv', usecols=[
                      'SK_ID_CURR'])  # import colonne id de test

# tornado client limited to 100mb

url = "https://test-deploiement-fastapi-v1.azurewebsites.net/"
df = pd.DataFrame()
dict_credit = {0: 'remboursé',
               1: 'non remboursé'}
# source = ColumnDataSource(df)
# ,align ='center'on ne met rien au départ # 100005 feature_importance = 0 pk ?
text = PreText(text=(""), styles={'font-size': '20pt', 'color': 'red'})
...
def upload_data(attr, old, new):  # charger et envoyer df fastapi nettoya
    ...
    df_retrieved[0] = pd.DataFrame(
        data_retrieved['df'])  # le dataframe dans source
...
def update_id(attr, old, new):  # attrname, old, new => obligatoire pour ce composant
    ...
    # si dataset nettoyé récupéré donc pas demo mais données réelles
    if (len(df_retrieved[0]) != 0) & (int(value_input) in df_retrieved[0]['SK_ID_CURR'].tolist()):
        print('df_remplie et id complété', value_input)
        df_filtered = df_retrieved[0][df_retrieved[0]
                                      ['SK_ID_CURR'] == int(value_input)]
        print('df avec id', df_filtered)
        res = requests.post(url+"predict/reel/",
                            json=df_filtered.to_json(orient='split'))
    ...
    # premiere ligne
    new_data['feature_importance'] = data_retrieved['feature_importance'][0]
    # sinon ne chnage qu'une information à la fois 0 index de la ligne
    new_data['colonnes'] = [i for i in range(
        len(data_retrieved['feature_importance'][0]))]
    # print(data_retrieved['nom_colonnes'])
    new_data['names'] = data_retrieved['nom_colonnes']
    source.data = new_data
    text.text = 'Le crédit sera {} pour l\'id {}'.format(
        dict_credit[data_retrieved['prediction']], value_input)  # 100057
    # print(new_data) #debugging si graph n'apparait pas
    positive_ = [True if float(
        imp) > 0 else False for imp in source.data['feature_importance']]
    negative_ = [False if float(
        imp) > 0 else True for imp in source.data['feature_importance']]
    neg_view.filter = BooleanFilter(negative_)
    view1.filter = BooleanFilter(positive_)


# quand la valeur est changée apres entrée ou click va dans la fonction update id
...
curdoc().add_root(column(row(auto_complete_input, file_input),
                         text, p, ph, ph_1))  # ,data_table
